...
`pnpm exec vp run test:m1:html` builds the native module, runs the positive `fixtures/m1-basic-html` product checks, verifies imported-file provenance and both committed compatibility results, and confirms the current rsvite outcome against the unchanged vendored test. It does not clone or install the external checkout or replace committed results. `pnpm exec vp run ready` depends on this focused task; a green run means the raw slice and recorded compatibility facts satisfy their contracts, not that rsvite passes the C2 upstream case.
<!-- Author: rsvite-lead -->
...
Before a recorder touches an external checkout or writes evidence, it settles that identity, removes the native loader and this host's binary, runs the repository's supported native build without cache, and resolves the product source completely again — protected main can advance while a build runs, including back to identical bytes. It then requires the loader and the exact binary this host's loader selects, both canonical regular files inside `packages/rsvite`, and resolves the command from that package's own `bin`. `NAPI_RS_NATIVE_LIBRARY_PATH` is refused, because the generated loader prefers it over every package-local binary. A recording proves its prepared subject is still current immediately before publishing, since preparation and publication are minutes apart. `@rsvite/compatibility-rsvite-workspace` owns this boundary for every recorder; the generic runner and the individual project adapters do not.
<!-- Author: rsvite-senior-engineer -->
